...
_zUnitsCivSer   = ["C_man_pilot_F","C_journalist_F","C_Marshal_F"];
...
while {true} do {
    {
        ...
        _spawnZpoints = nearestObjects [_thisPlayer,["Building"], zSpawnRange];
        _spawnZpointsCount = count _spawnZpoints;

        _zCount = floor (_spawnZpointsCount/2);
        ...
        if (count (units groupZ) < _zCount) then {
            _rdmLocPos = getPos (selectRandom _spawnZpoints);
            ...
            if([objNull, "VIEW"] checkVisibility [eyePos _thisPlayer, _pos] == 0 && _thisPlayer distance _pos > 30 && _inViewAngle < 120)then{
                ...
                _z = selectRandom [selectRandom _zUnitsCiv,selectRandom _zUnitsMilInf];
