// Weapons
_sortedWeaponList = [] call bde_fetchWeapons;
lightWeapons     = _sortedWeaponList select 0; // Light Damage Weapons;
mediumWeapons    = _sortedWeaponList select 1; // Medium Damage Weapons;
heavyWeapons     = _sortedWeaponList select 2; // Heavy Damage Weapons;
        ...
        for "_i" from 0 to _posBuilPos step 1 do {
          ...
          switch(_buildingType) do {
            case "MilitaryBuilding":{
              ...
              /*/ Ammo for Pistols
              if(random 100 < 30) then {
                _rdmPistol = selectRandom _weaponPistolsMil;
                _pmags = getArray(configfile >> "cfgWeapons" >> _rdmPistol >> "magazines");
                _spawn addMagazineCargoGlobal [selectRandom _pmags, round(random 3)+2];
              };
              // Ammo for Rifles
              if(random 100 < 30) then {
                _rdmRifle = selectRandom _weaponRiflesMil;
                _rmags = getArray(configfile >> "cfgWeapons" >> _rdmRifle >> "magazines");
                _spawn addMagazineCargoGlobal [selectRandom _rmags, round(random 2)+2];
              };*/

              // Ammo only
              if(random 100 < 30) then {
                _rdmWeapon  = selectRandom heavyWeapons;
                _rmags      = getArray(configfile >> "cfgWeapons" >> _rdmWeapon >> "magazines");
              ...
              if(random 100 < 20) then {
                ...
                _pmags = getArray(configfile >> "cfgWeapons" >> _rdmPistol >> "magazines");
                _spawn addMagazineCargoGlobal [selectRandom _pmags, round(random 3)];
              };
              // Rifles
              if(random 100 < 30) then {
                _rdmRifle = selectRandom _weaponRiflesMil;
                ...
                _rmags = getArray(configfile >> "cfgWeapons" >> _rdmRifle >> "magazines");
                _spawn addMagazineCargoGlobal [selectRandom _rmags, round(random 2)];
              };*/
            ...
            case "AirportBuilding":{
                // Ammo only
                if(random 100 < 30) then {
                  _rdmWeapon  = selectRandom mediumWeapons;
                  _rmags      = getArray(configfile >> "cfgWeapons" >> _rdmWeapon >> "magazines");
                  _spawn addMagazineCargoGlobal [selectRandom _rmags, round(random 3)+1];
                };
            ...
            case "CivilianBuilding":{
              ...
              /*/ Ammo for Pistols
              if(random 100 < 10) then {
                _rdmPistol = selectRandom _weaponPistolsCiv;
                _pmags = getArray(configfile >> "cfgWeapons" >> _rdmPistol >> "magazines");
                _spawn addMagazineCargoGlobal [selectRandom _pmags, round(random 3)+1];
              };
              // Ammo for Rifles
              if(random 100 < 10) then {
                _rdmRifle = selectRandom _weaponRiflesCiv;
                _rmags = getArray(configfile >> "cfgWeapons" >> _rdmRifle >> "magazines");
                _spawn addMagazineCargoGlobal [selectRandom _rmags, round(random 2)+1];
              };*/

              // Ammo only
              if(random 100 < 30) then {
                _rdmWeapon  = selectRandom lightWeapons;
                _rmags      = getArray(configfile >> "cfgWeapons" >> _rdmWeapon >> "magazines");
                _spawn addMagazineCargoGlobal [selectRandom _rmags, round(random 3)+1];
              };
